...
//    ofstream out_eq;
//    out_eq.open("equilibration_gas.dat");
    for (int i=0; i<100000; i++) {
        Move();
//        Measure();
//        out_eq << i << setw(12) << walker[it] << endl;
    }
    ConfFinal();
//    out_eq.close();
    r=1;
...
void Input(void)
    ...
    //se iNVET = 0 allora fa MD, altrimenti fa MC. 1.1 T, poi c'è numero di particelle, poi densità, poi cut off, poi 0.0005 è il timestep, 20 blocchi di 2000 step ciascuno
    ReadInput >> iNVET;
    ReadInput >> restart;
...
void Measure() //Properties measurement
{
    ...
    walker[ip] = 48.0/(3.0*vol) * p + rho*walker[it];  // Pressure (sono giuste le unità naturali?)
...
void Averages(int iblk) //Print results for current block
{
    ...
    P.open("output_p_gas.dat",ios::app);
    ...
    //Total energy
    Etot << setprecision(8) << setw(wd) << iblk <<  setw(wd) << stima_etot << setw(wd) << glob_av[ie]/(double)iblk << setw(wd) << err_etot << endl;
    //Temperature
    Temp << setw(wd) << iblk <<  setw(wd) << stima_temp << setw(wd) << glob_av[it]/(double)iblk << setw(wd) << err_temp << endl;
    //Pressure
    P << setw(wd) << iblk <<  setw(wd) << stima_p << setw(wd) << glob_av[ip]/(double)iblk << setw(wd) << err_p << endl;
